...
################################################################
# QRCode generator
# from QRCoder (https://github.com/codebude/QRCoder), Copyright (c) 2013-2018 Raffael Herrmann
# modified by SATO Yusuke (https://github.com/satob)
################################################################
...
namespace QRCoder.Exceptions
{
    public class DataTooLongException : Exception
    {
        public DataTooLongException(string eccLevel, string encodingMode, int maxSizeByte) : base(
            String.Format("The given payload exceeds the maximum size of the QR code standard. The maximum size allowed for the choosen paramters (ECC level={0}, EncodingMode={1}) is {2} byte.", eccLevel, encodingMode, maxSizeByte)
        ){}
    }
}
...
Get-ChildItem "$BaseName.*" | ForEach-Object {
    $FileName = $_.FullName;
    $Content = Get-Content -Encoding Byte $_.FullName
    $Base64Content = [Convert]::ToBase64String($Content)
    $Base64Content = "begin-base64 644 " + $_.Name + "`n" + $Base64Content + "`n===="

# $Base64Content | Set-Content -Encoding ASCII "$_.FullName.b64.txt"

    $QRCodeData = $QRGenerator.CreateQrCode($Base64Content, [QRCoder.ECCLevel]::L);
